...
function placePotholes(currentPotholes, userType) {
    ...
    for (var i = 0; i < currentPotholes.length; i++) {
        ...
        var contentString = '<form action="/function/update" method="post"><select name="severity"><option value="0">Severity</option><option value="1">1</option><option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5">5</option></select><br /><input name="PotholeID" type="hidden" value="' + currentPotholes[i].PotholeId + '"> <button id="inspectButton" type="submit">Mark For Repair</button> </form>';
